...
public class DungeonRoomRegistry {
	...
	private static final class DungeonRoomList {
		...
		public void add(DungeonRoomRecord record) {
			if (recordList.contains(record)) {
				...
				AutoDungeons.LOGGER.info("Overriding DungeonRoomList registration for entry " + record.room.getRoomID());
	...
	@Nullable
	public DungeonRoomRecord getRandomRoom(Random rand) {
		return getRandomRoom(rand, INTERNAL_ALL_NAME);
	}
	
	@Nullable
	public DungeonRoomRecord getRandomRoom(Random rand, String tag) {
		DungeonRoomRecord ret = null;
		DungeonRoomList list = tagMap.get(tag);
		if (list != null) {
			int idx = rand.nextInt(list.weightSum);
			for (DungeonRoomRecord record : list.recordList) {
				idx -= record.weight;
				if (idx < 0) {
					ret = record;
